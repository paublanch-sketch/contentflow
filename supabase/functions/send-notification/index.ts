...
// Envía emails via Gmail SMTP (App Password) — gratis, ~500/día, sin registros externos
//
...
Deno.serve(async (req) => {
  ...
  try {
    const {
      type,           // 'Aprobado ✅' | 'Cambios solicitados ⚠️'
      client_name,
      post_number,
      platform,
      copy_preview,
...
<body style="font-family:Arial,sans-serif;background:#f4f4f4;margin:0;padding:20px">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1)">
    ...
    await transporter.sendMail({
      from:    `"FlowAPP" <${GMAIL_USER}>`,
      to:      GMAIL_USER,   // te lo mandas a ti mismo
      subject,
      html,
    });
